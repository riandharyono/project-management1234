...
export function TaskCard({ task, members, labels, onOpen, onQuickMenu, stage = "todo" }) {
  ...
  return (
    <article className="kb-card" onClick={onOpen} data-testid={`task-card-${task.id}`}>
      <span className={`kb-stage-badge ${stage}`} data-testid={`task-stage-badge-${task.id}`} title={{ done: "Selesai", cancelled: "Dibatalkan", todo: "Belum dikerjakan", progress: "Sedang dikerjakan" }[stage]}>
        <StageIcon size={14} />
      </span>
      {onQuickMenu && (
